...
(function(){
  ...
  (function(){
    ...
    document.querySelectorAll("[data-stagger]").forEach(group=>{
      Array.from(group.children).forEach((child, i)=>{
        if(child.classList.contains("reveal")) child.dataset.delay = (i*60).toString();
      });
    });
  ...
  document.querySelectorAll(".arch-opt").forEach(o=>{
    ...
    o.addEventListener("keydown",(e)=>{ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); applyConfig(o.dataset.key); } });
  });
  applyConfig("both");
  ...
  document.querySelectorAll(".claim-cta").forEach(btn=>{
    btn.addEventListener("click",()=>{
      ...
      btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12l5 5 9-11"/></svg> Added, credit applied';
      clearTimeout(btn._t);
      btn._t = setTimeout(()=>{ btn.innerHTML = orig; }, 1800);
    });
  });
  ...
  function wireVideo(videoId, overlayId){
    ...
    if(!v || !overlay) return;
    overlay.addEventListener("click", ()=>{ const p=v.play(); if(p&&p.catch) p.catch(()=>{}); });
  ...
  document.querySelectorAll(".faq-item").forEach(item=>{
    ...
    q.addEventListener("click",()=>{
      ...
      document.querySelectorAll(".faq-item").forEach(x=>{x.classList.remove("open");const xq=x.querySelector(".faq-q");if(xq)xq.setAttribute("aria-expanded","false");});
  ...
  (function(){
    ...
    function onScroll(){
      ...
      if(past && beforeFooter){ sticky.classList.add("show"); document.body.classList.add("pad-sticky"); }
      else { sticky.classList.remove("show"); document.body.classList.remove("pad-sticky"); }
    }
  ...
  (function(){
    ...
    thumbs.forEach(t=>{
      t.addEventListener("click",()=>{
        ...
        setTimeout(()=>{ main.innerHTML = '<img src="'+src+'" alt="Revive Veneers, '+alt+'" />'; main.classList.remove("fade"); }, 170);
      });
    });
  })();
